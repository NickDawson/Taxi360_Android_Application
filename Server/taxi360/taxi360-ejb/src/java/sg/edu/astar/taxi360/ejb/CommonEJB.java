...
//        if (passenger != null && passenger.getAccesskey() == null) {
    ...
    public Boolean logoutPassenger(Passenger passenger) {
        Passenger p = passengerFacade.find(passenger.getId());
        if (p != null || p.getRegistrationtype().equalsIgnoreCase("normal")) {
            p.setAccesskey("");
    ...
    public Boolean logoutDriver(Driver driver) {
        Driver d = driverFacade.find(driver.getId());
        if (d != null) {
            d.setAccesskey("");
            driverFacade.edit(d);
        }
    ...
    public Boolean validateAccessKey(String key) {
        return passengerFacade.validateAccessKey(key) || driverFacade.validateAccessKey(key);
    }
