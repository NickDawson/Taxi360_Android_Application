...
public class RequestFacade extends AbstractFacade<Request> {
    ...
    public List<Request> findWithinRange(Location center, Location minLoc, Location maxLoc, Double radius) {
        System.out.println("location="+center+", minLoc="+maxLoc+",minLoc="+maxLoc+", higher=" + radius);
        //String sql = "Select request.* from request where ST_Distance(location, ST_GeographyFromText('SRID=4326;POINT( "+center.getLongitude()+" "+center.getLatitude()+")')) BETWEEN "+lower+" AND "+higher+"";
        String sql = "Select request.* from request where ST_DWithin(location, ST_GeographyFromText('SRID=4326;POINT( "+center.getLongitude()+" "+center.getLatitude()+")'),"+radius+") AND NOT(location && ST_MakeEnvelope("
                +minLoc.getLongitude()+","+minLoc.getLatitude()+","+maxLoc.getLongitude()+","+maxLoc.getLatitude()+","+"4326"+"))";
        System.out.println("sql stmt= "+sql);
        Query query = em.createNativeQuery(sql, Request.class);
        return query.getResultList();
    }
