...
public class PassengerFacade extends AbstractFacade<Passenger> {
    ...
    public Boolean validateAccessKey(String key){
        try {
            TypedQuery<Passenger> tq = em.createNamedQuery("Passenger.validateAccessKey", Passenger.class);
            tq.setParameter("accesskey", key);
            return tq.getSingleResult().getAccesskey().equals(key);
        } catch (NoResultException e) {
            return false;
        }
